...
> **⚠️ Windows 用户请注意**
>
> tmux 在 Windows 上默认使用 cmd.exe，可能有显示问题。扩展模块加载时自动写入
> `~/.tmux.conf` 配置，但如果遇到问题，请手动检查配置文件内容：
>
> ```tmux
> # ~/.tmux.conf — 路径必须用正斜杠 /，不能用 \（tmux 把 \ 当转义字符）
> set -g mouse on
> set -g default-shell "C:/Program Files/WindowsApps/Microsoft.PowerShell_7.*/pwsh.exe"
> ```
>
> **备选方案**：若 WindowsApps 权限导致 pwsh 无法启动，改用 Git Bash：
>
> ```tmux
> set -g default-shell "C:/Program Files/Git/bin/bash.exe"
> ```
>
> 修改后退出 tmux 重新 `tmux new-session -s pi` 生效。详见[常见问题](#常见问题)。
...
### Windows 上 tmux 默认打开 cmd，怎么改为 PowerShell 或 Bash？

tmux 的 `default-shell` 配置项控制新 session/window 使用什么 shell。
Windows 上的注意事项：

1. **必须用完整路径** — 不能用 `"pwsh"` 之类的短名，tmux 会报 `not a suitable shell: pwsh`
2. **必须用正斜杠 `/`** — tmux 配置文件的 `\` 当作转义字符，`C:\Program Files\...` 会变成 `C:Program Files...`
3. **推荐配置**：扩展已自动写入 `~/.tmux.conf`，但如果需要手动修改：
...
修改后退出 tmux 重新 `tmux new-session -s pi` 生效。

如果 WindowsApps 权限导致 pwsh 无法启动，`git bash` 是可靠的备选。
