...
> `~/.tmux.conf` 配置（**优先检测 Git Bash**），但如果遇到问题，请手动检查：
>
> ```tmux
...
### Windows 上 tmux 默认打开 cmd，怎么改为 Bash？

tmux 的 `default-shell` 配置项控制新 session/window 使用什么 shell。
Windows 上的注意事项：

1. **必须用完整路径** — 不能用 `"bash"` 之类的短名
2. **必须用正斜杠 `/`** — tmux 配置文件把 `\` 当转义字符，反斜杠路径会被吞掉
3. **推荐 Git Bash**（最稳定，无显示问题）：

```tmux
set -g default-shell "C:/Program Files/Git/bin/bash.exe"
```

扩展会自动检测并写入 `~/.tmux.conf`。修改后退出 tmux 重新 `tmux new-session -s pi` 生效。
